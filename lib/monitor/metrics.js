...
/**
 * Adds the average rate over the stream's whole life.
 *
 * Drive delivers in bursts — it fills the read-ahead buffer, stops while the
 * player drains it, then fills again — so an instantaneous rate sampled during
 * a pause reads zero on a perfectly healthy stream. It was doing exactly that
 * on the first deployment and made the Drive hop look dead. The average over
 * the whole stream is the number that can be compared against a file's bitrate
 * and mean something; the instantaneous one is kept beside it for spotting a
 * stall as it happens.
 */
function withAverages(record) {
  const until = record.endedAt ?? Date.now();
  const seconds = Math.max(1, (until - record.startedAt) / 1000);
  return {
    ...record,
    elapsedSeconds: seconds,
    driveAverage: record.driveBytes / seconds,
    clientAverage: record.clientBytes / seconds,
  };
}

export function activeStreams() {
  return [...store().active.values()].map(withAverages);
}

export function finishedStreams() {
  return store().finished.toArray().map(withAverages);
}
...
/**
 * Pages moved between RAM and disk since boot.
 *
 * This, not swap occupancy, is what says the box is in trouble. Occupancy only
 * says pages were moved out at some point — a long build does that and the
 * pages then sit there untouched, costing nothing. It made the dashboard
 * report a swapping box on an idle one with 1.2 GB free. Paging *rate* is the
 * thing that stalls reads, so that is what gets measured.
 */
async function swapRates() {
  try {
    ...
    const pin = grab('pswpin');
    const pout = grab('pswpout');
    if (pin === null || pout === null) return null;
    return { pagesIn: pin, pagesOut: pout };
  } catch {
    return null;
  }
}
...
/**
 * Swap in use. Kept because a swap file filling up is still worth seeing, but
 * read alongside the paging rate above rather than on its own.
 */
...
async function sample() {
  ...
  // Pages actually moving to or from disk, per second. Anything sustained
  // here is the classic 2 GB failure: reads queue behind disk IO and playback
  // stutters while CPU and bandwidth both look idle. A swap file merely
  // holding pages from an earlier build is not that, and no longer reported
  // as if it were.
  const paging = await swapRates();
  let swapPagesPerSec = 0;
  if (paging && s.lastPaging) {
    const seconds = (now - s.lastPaging.at) / 1000;
    if (seconds > 0) {
      swapPagesPerSec =
        Math.max(0, paging.pagesIn - s.lastPaging.pagesIn) / seconds +
        Math.max(0, paging.pagesOut - s.lastPaging.pagesOut) / seconds;
    }
  }
  if (paging) s.lastPaging = { ...paging, at: now };

  // Once when it crosses, so a permanently-swapping box does not fill the ring.
  const thrashing = swapPagesPerSec > 100;
  if (thrashing && !s.swapWarned) {
    s.swapWarned = true;
    recordEvent('warn', 'the box is paging to disk — reads are queueing behind IO', {
      swapPagesPerSec: Math.round(swapPagesPerSec),
    });
  } else if (!thrashing) {
    s.swapWarned = false;
  }
