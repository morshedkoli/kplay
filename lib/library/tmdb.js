...
export async function searchMovie(title, year) {
  ...
  // TMDb's `year` param filters strictly on primary release year, which can
  // differ from the filename's year (festival vs wide release, region). Retry
  // without it rather than surface a false unmatched.
  if (!hit && year) {
    const retry = await get('/search/movie', { query: title });
    hit = retry.results?.[0];
  }
  if (!hit) return null;
  return {
