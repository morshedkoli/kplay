...
export async function createIndexes(db) {
  ...
  // sourceUrl is the DhakaFlix equivalent of driveFileId: the identity a scan
  // dedupes on. Unique so a re-crawl cannot import the same file twice, sparse
  // so every Drive-backed doc (which has no sourceUrl at all) stays out of it.
  created.push(
    await db
      .collection('media')
      .createIndex({ sourceUrl: 1 }, { unique: true, sparse: true, name: 'sourceUrl_unique' })
  );
  created.push(
    await db
      .collection('episode')
