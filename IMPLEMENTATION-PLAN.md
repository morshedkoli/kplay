# kPlay: Serverless Migration — Implementation Plan

> **Portable plan.** Self-contained for any IDE agent (Cursor, Copilot, Windsurf, Cline, Claude Code, or a human). Work top to bottom; each task is a checklist of 2–5 minute steps ending in a commit. Tick `- [ ]` boxes as you go. No external tooling or plugins required.
...
**Design doc:** `docs/superpowers/specs/2026-09-07-serverless-drive-direct-design.md` — read it before Task 1. It is the authority; where this plan and the design doc disagree, the design doc wins.

---

## Why this exists

The VPS currently proxies every video byte: Drive → VPS → player. A film the
server never needs to inspect consumes the server's bandwidth, and the hosting
plan's data cap is the binding limit on how much can be watched.

`app/api/media/play-url/[id]/route.js` already solves this for the Android TV
client, which reads bytes straight from Drive. Browsers still proxy, and the VPS
exists only to serve them. These thirteen tasks close that gap and delete the
server.

**Done means:** a full film plays end to end while Vercel's usage dashboard
shows no measurable data transfer, and the VPS can be cancelled.

## Orientation for someone new to this repo

One full-stack Next.js project at the repo root — the same app serves the web UI
and the `/api/media/*` routes. One process, one origin, no CORS, no separate
backend. Single Google account, single Drive, single shared library.

| Path | What lives there |
|---|---|
| `app/` | Next.js App Router — pages and API routes together |
| `app/api/media/` | The catalog and playback API |
| `app/admin/` | Admin screens, gated by `lib/admin-auth.js` |
| `lib/` | All logic worth testing. **The test runner only sees this directory.** |
| `lib/library/` | Filename parsing, TMDb matching, mkv indexing |
| `lib/models/media.js` | The `media` and `episode` Mongo collection shapes |
| `lib/gdrive.js` | The only place Drive is touched |
| `scripts/` | Local jobs, not deployed |
| `android-tv/` | The Android TV client, a separate Gradle project |
...
npm run migrate    # create Mongo indexes
npm run test:db    # connectivity check against MONGODB_URI
```

**Environment** — copy `.env.example` to `.env.local`. The names that matter
here: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REFRESH_TOKEN`,
`GOOGLE_DRIVE_FOLDER_ID`, `TMDB_API_KEY`, `MONGODB_URI`, `ADMIN_PASSWORD`,
`KDRIVE_DEVICE_KEY`.

## How to work through this

1. Read the design doc, then this plan's Global Constraints below.
2. Do Task 1 first and honestly. It is a measurement, and its result decides
   whether Tasks 10 and 13's proxy deletion happen at all.
3. Then go in order. Tasks 2–10 are code and can be done in one sitting;
   Tasks 11–13 need your Vercel, Atlas, and Google accounts and a device.
4. Commit at the end of every task, using the message given in its final step.
5. Run `npm test` before each commit that touches `lib/`.

Every task states the exact files, the exact test code, and the exact
implementation. Where a step says to check something against the real repo
(`grep` steps), do that check — the plan was written against the code as it
stood and a signature may have moved.
...
## Coverage notes (from the plan review)
